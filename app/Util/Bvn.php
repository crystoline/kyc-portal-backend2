...
class Bvn
{
    ...
    public function __construct()
    {
        ...
        $this->user_name = 'capricon@upperlink.ng'; //setting('bvn_user_id', 'capricon@upperlink.ng');
        $this->password = 'test'; //setting('bvn_password', 'test');
    }
    public static function post($url, array $data, $action_name, $headers = array())
    {
        ...
        try {
            ...
            if ($response_code === 200) {
                ...
                Log::info("Success ({$response_code}): occurred: \nMethod: {$action_name} \nUrl: {$url}\nHeaders: {$headers_string} \nData: {$data_string}\nResponse {$content}");
    ...
    public function login (): string
    {
        ...
        return Cache::get(self::CACHE_KEY, static function () use ($url, $data) {



            $response = self::post($url, $data, __METHOD__);
            if(isset($response['data']['token'])){
                Cache::put(self::CACHE_KEY, $response['data']['token'], Carbon::now()->addMinutes(30));
