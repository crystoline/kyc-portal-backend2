...
class Sql{
    ...
    {
        if(method_exists($this, 'get' .Str::studly($method))){
            $m = 'get' .Str::studly($method);
            $result = $this->$m($args);
        }

        if(isset($result)) return $result[$this->driver] ?? $result['mysql'];
    ...
    function __get($attribute)
    {
        ...
        if(isset($result[$this->driver])) {
            return $result[$this->driver];
        }

        return null;
    }

    public function getDateFormat($args): array
    {
        return [
    ...
    public function getDateAdd($args): array
    {
        return array(
    ...
    public function getNullsLast($args): array
    {
        return [
    ...
    public function specificOrder($column,$array=[]): string
    {
        if(empty($array)) {
            return $column;
        }
    ...
    public function tableComment($query): ?bool
    {
        switch($this->driver){
            case 'sqlsrv':
                return null;
    ...
    public function alterEnumColumn($table, $column, array $enums, $default = null, $comment = "", $after = ""): ?bool
    {
    ...
    public function alterTable($table, $column, $column_properties){
        switch($this->driver){
            case 'sqlsrv':
    ...
    public function resetAutoIncrement($table): ?bool
    {
        switch($this->driver){
            case 'sqlsrv':
                return null;
    ...
    public function groupByYearMonth($column='created_at'): ?string
    {
        switch($this->driver){
            case 'sqlsrv':
    ...
    public function groupByYear($column='created_at'): ?string
    {
        switch($this->driver){
            case 'sqlsrv':
    ...
    public function dropDefaultConstraint($table,$column): bool
    {
